...
from app.modules.inventory.schemas.category import CategoryCreate, CategoryResponse, CategoryUpdate
...
def update_category(category_id: int, data: CategoryUpdate, db: Session = Depends(get_db)) -> CategoryResponse:
    ...
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
    return service.update(db, category=category, name=data.name)
